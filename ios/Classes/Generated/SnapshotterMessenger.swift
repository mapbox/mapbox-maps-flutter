...
struct MapSnapshotOptions {
  ...
  static func fromList(_ __pigeon_list: [Any?]) -> MapSnapshotOptions? {
    let size = __pigeon_list[0] as! Size
    let pixelRatio = __pigeon_list[1] as! Double
    let glyphsRasterizationOptions: GlyphsRasterizationOptions? = nilOrValue(__pigeon_list[2])
    let showsLogo: Bool? = nilOrValue(__pigeon_list[3])
    let showsAttribution: Bool? = nilOrValue(__pigeon_list[4])
