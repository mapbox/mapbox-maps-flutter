...
protocol NavigationInterface {
  fun setRoute(waypoints: List<Point>) throws
  fun stopTripSession() throws
  fun startTripSession(withForegroundService: Boolean) throws
  fun requestNavigationCameraToFollowing() throws
  fun requestNavigationCameraToOverview() throws
  fun lastLocation() throws
}
...
class NavigationInterfaceSetup {
  ...
  static func setUp(binaryMessenger: FlutterBinaryMessenger, api: LogoSettingsInterface?, messageChannelSuffix: String = "") {
    ...
    if let api = api {
      let args = message as! [Any?]
      let waypointsArg = args[0] as! List<Point>
      setRouteChannel.setMessageHandler { _, reply in
        do {
          let result = try api.setRoute(waypointsArg)
          reply(wrapResult(result))
        } catch {
          reply(wrapError(error))
        }
    ...
    if let api = api {
      stopTripSessionChannel.setMessageHandler { message, reply in
        do {
          try api.stopTripSession()
          reply(wrapResult(nil))
        } catch {
          reply(wrapError(error))
        }
