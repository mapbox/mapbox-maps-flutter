...
Future<MapboxMap> _setupMap(
  ...
  final mapboxMap = await app.pumpMap(tester: tester, styleUri: styleURI);
  await tester.pumpAndSettle();
  if (!app.events.onStyleLoaded.isCompleted) {
    await app.waitForEvent(
      tester,
      app.events.onMapLoaded.future,
      description: () => 'map never finished loading $styleURI',
    );
  }
  return mapboxMap;
}

/// Helper: mount the map with STANDARD loaded (provides the `basemap`
/// import). `_setupMap` already mounts a fresh map that loads STANDARD; do
/// NOT reload it here — gl-js `setStyle` diffs against the current style, so
/// re-loading the style that is already active is a no-op that never fires
/// `styleLoaded`, and awaiting it hangs the test forever.
Future<MapboxMap> _setupMapWithStandard(WidgetTester tester) async {
  final mapboxMap = await _setupMap(tester);
  if (!app.events.onStyleLoaded.isCompleted) {
    await app.waitForEvent(
      tester,
      app.events.onStyleLoaded.future,
      description: () => 'STANDARD style never finished loading',
    );
  }
  await tester.pumpAndSettle();
  return mapboxMap;
...
void main() {
  ...
  patrolTest('addStyleImportFromJSON without config or position', ($) async {
    final tester = $.tester;
    final mapboxMap = await _setupMap(tester);
  ...
  patrolTest('addStyleImportFromJSON with config and position', ($) async {
    final tester = $.tester;
    final mapboxMap = await _setupMap(tester);
  ...
  patrolTest('addStyleImportFromURI without config', ($) async {
    final tester = $.tester;
    final mapboxMap = await _setupMap(tester);
    ...
    // web, reading imports serializes the style, which throws "Style is not
    // done loading" until the reload finishes, so wait for the new MapLoaded.
    if (kIsWeb) {
  ...
  patrolTest('addStyleImportFromURI with importPosition below', ($) async {
    final tester = $.tester;
    final mapboxMap = await _setupMap(tester);
    ...
    await app.waitForEvent($.tester, app.events.onStyleDataLoaded.future);
  ...
  patrolTest('updateStyleImportWithJSON data-only', ($) async {
    final tester = $.tester;
    final mapboxMap = await _setupMap(tester);
  ...
  patrolTest('updateStyleImportWithJSON data+config', ($) async {
    final tester = $.tester;
    final mapboxMap = await _setupMap(tester);
  ...
  patrolTest('updateStyleImportWithURI data-only', ($) async {
    final tester = $.tester;
  ...
  patrolTest('updateStyleImportWithURI data+config', ($) async {
    final tester = $.tester;
    final mapboxMap = await _setupMap(tester);
  ...
  patrolTest('moveStyleImport above', ($) async {
    final tester = $.tester;
    final mapboxMap = await _setupMap(tester);
  ...
  patrolTest('moveStyleImport below', ($) async {
    final tester = $.tester;
    final mapboxMap = await _setupMap(tester);
  ...
  patrolTest('moveStyleImport at', ($) async {
    final tester = $.tester;
    final mapboxMap = await _setupMap(tester);
  ...
  patrolTest('moveStyleImport null position is a no-op on web', ($) async {
    final tester = $.tester;
    final mapboxMap = await _setupMap(tester);
  ...
  patrolTest('removeStyleImport success path', ($) async {
    final tester = $.tester;
    final mapboxMap = await _setupMap(tester);
  ...
  patrolTest('getStyleImportSchema returns schema with known keys', ($) async {
    final tester = $.tester;
  ...
  patrolTest('getStyleImportConfigProperty single read', ($) async {
    final tester = $.tester;
    final mapboxMap = await _setupMapWithStandard(tester);
  ...
  patrolTest('getStyleImportConfigProperties bulk read', ($) async {
    final tester = $.tester;
    final mapboxMap = await _setupMapWithStandard(tester);
  ...
  patrolTest('setStyleImportConfigProperty single write round-trips', (
    $,
  ) async {
    final tester = $.tester;
    final mapboxMap = await _setupMapWithStandard(tester);
  ...
  patrolTest('setStyleImportConfigProperties bulk write round-trips', (
    $,
  ) async {
    final tester = $.tester;
    final mapboxMap = await _setupMapWithStandard(tester);
