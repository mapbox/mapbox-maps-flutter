...
void main() {
  ...
  patrolTest('Location is updated puck 2d', ($) async {
    final tester = $.tester;
    final mapboxMap = await app.pumpMap(tester: $.tester);
    await tester.pumpAndSettle();
    final location = mapboxMap.location;
    // The low-zoom stop must stay above 0. The map defaults to zoom 0, so a
    // stop of 0 scales the puck to nothing, and rasterizing an icon at scale 0
    // throws inside the renderer.
    // https://mapbox.atlassian.net/browse/MAPSNAT-4152
    final expression = '["interpolate",["linear"],["zoom"],8,0.5,24,1]';
    ...
    // Enabling the location component makes iOS show the system location
    // permission dialog, which wedges this and every following test on the
    // device farm (the old suite dodged it by running location tests last).
    // Grant it through Patrol's native automator. Android needs no handling:
    // the component doesn't request the runtime permission there.
    if (isIOS &&
    ...
    expect(
      updatedSettings.accuracyRingBorderColor,
    ...
    //     settings.locationPuck?.locationPuck2D?.bearingImage);
    expect(
      updatedSettings.locationPuck?.locationPuck2D?.scaleExpression,
  ...
  patrolTest('Location is updated puck 3d', ($) async {
    final tester = $.tester;
    final mapboxMap = await app.pumpMap(tester: $.tester);
    await tester.pumpAndSettle();
    ...
    expect(
      updatedSettings.locationPuck?.locationPuck3D?.modelUri,
      settings.locationPuck?.locationPuck3D?.modelUri,
    );
    expect(
      updatedSettings.locationPuck?.locationPuck3D?.position,
      settings.locationPuck?.locationPuck3D?.position,
    );
    expect(
      updatedSettings.locationPuck?.locationPuck3D?.modelOpacity,
    ...
    expect(
      updatedSettings.locationPuck?.locationPuck3D?.modelScaleExpression,
