...
// packages/mapbox_maps_flutter/test/annotation/point_annotation_manager_events_test.dart
void main() {
  setUpAll(() => MapboxOptions.setAccessToken(ACCESS_TOKEN));

  patrolTest('PointAnnotationManager custom id and position', skip: kIsWeb, (
    $,
  ) async {
    final tester = $.tester;
  ...
  patrolTest('create PointAnnotation_manager ', skip: kIsWeb, ($) async {
    final tester = $.tester;
    final mapboxMap = await app.pumpMap(tester: $.tester);
    await tester.pumpAndSettle();
    final manager = await mapboxMap.annotations.createPointAnnotationManager();
