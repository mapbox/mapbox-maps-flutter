...
class RainbowRoadExample extends StatefulWidget {
...
class RainbowRoadExampleState extends State<RainbowRoadExample> {
  ...
  Future<void> _onStyleLoadedCallback(StyleLoadedEventData data) async {
    ...
    await mapboxMap.addLayer(
      LineLayer(
        ...
        lineColor: Colors.grey.shade800.value,
        lineWidth: 4.0,
        lineBorderWidth: 5.0,
  ...
  Future<void> _frameTrack(MapboxMap mapboxMap, String trackGeoJson) async {
    final coordinates =
        (json.decode(trackGeoJson)['features'][0]['geometry']['coordinates']
                as List)
            .map(
              (c) => Point(
  ...
  void _startAnimation() {
    ...
    _animationTimer = Timer.periodic(const Duration(milliseconds: 50), (timer) {
      ...
      mapboxMap.setStyleLayerProperty(
        _layerId,
...
List<Object> _rainbowExpression(double phase) {
  ...
  for (var i = 0; i <= stopCount; i++) {
    ...
    stops.add(t);
    stops.add(['rgb', color.red, color.green, color.blue]);
  }
  return [
    'interpolate',
    ['linear'],
