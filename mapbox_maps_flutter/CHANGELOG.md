...
* Add `MapboxMap.location.setExternalLocation` and `.clearExternalLocation`, letting apps drive the location puck from a source other than the platform's default GPS-based provider — an indoor-positioning SDK, a simulation, or a vehicle's own sensors. The native location-provider override is registered lazily on the first `setExternalLocation` call, so puck behaviour is unchanged until it is used, and `clearExternalLocation` restores the default provider. `floor` is applied on iOS only, since Android's `LocationConsumer` API has no floor concept. Android and iOS only; throws `UnsupportedError` on web, where GL JS's `GeolocateControl` owns the puck position and exposes no override. Resolves [#1085](https://github.com/mapbox/mapbox-maps-flutter/issues/1085).
