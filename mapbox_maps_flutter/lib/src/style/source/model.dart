...
class ModelSourceModel {
  ...
  static ModelSourceModel fromJson(String id, Map<String, dynamic> json) {
    return ModelSourceModel(
      id: id,
      featureProperties: (json["featureProperties"] as Map?)
          ?.cast<String, dynamic>(),
      materialOverrideNames: (json["materialOverrideNames"] as List?)?.cast(),
      materialOverrides: (json["materialOverrides"] as Map?)?.entries
          .map(
            (entry) => ModelMaterialOverride.fromJson(
              entry.key as String,
      ...
      nodeOverrides: (json["nodeOverrides"] as Map?)?.entries
          .map(
            (entry) => ModelNodeOverride.fromJson(
              entry.key as String,
              Map<String, dynamic>.from(entry.value as Map),
            ),
          )
...
class ModelNodeOverride {
  ModelNodeOverride({required this.name, this.orientation});
