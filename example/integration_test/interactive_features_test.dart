...
void main() {
  ...
  /// Tests using .dispatch() can only run on Android until it is exposed for iOS
  if (Platform.isAndroid) {
    testWidgets('test_addLongTapInteractionToMap', (WidgetTester tester) async {
      ...
      final mapFuture = app.main(
          ...
          camera: CameraOptions(
              center: Point(coordinates: Position(0, 0)), zoom: 10),
          alignment: Alignment(100, 100));
      await tester.pumpAndSettle();
      final mapboxMap = await mapFuture;
      var triggered = false;
    ...
    testWidgets('test_addTapInteractionToLayer', (WidgetTester tester) async {
      ...
      final mapboxMap = await mapFuture;
      var styleJson =
          await rootBundle.loadString('assets/featuresetsStyle.json');
      mapboxMap.style.setStyleJSON(styleJson);
      ...
      var point = await mapboxMap
          .pixelForCoordinate(Point(coordinates: Position(0, 0)));
    ...
    testWidgets('test_addLongTapInteractionToLayer',
        (WidgetTester tester) async {
      // load style and position camera
      final mapFuture = app.main(
          width: 200,
